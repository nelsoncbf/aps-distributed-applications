...
public class Thread06 {
    ...
    public static void main(String args[]) {
        ...
        ThreadSoma ts2 = new ThreadSoma(Math.floor(n/6), Math.floor(2*n/6));
        ThreadSoma ts3 = new ThreadSoma(Math.floor(2*n/6), Math.floor(3*n/6));
        ThreadSoma ts4 = new ThreadSoma(Math.floor(3*n/6), Math.floor(4*n/6));
        ThreadSoma ts5 = new ThreadSoma(Math.floor(4*n/6), Math.floor(5*n/6));
        ThreadSoma ts6 = new ThreadSoma(Math.floor(5*n/6), n);
        ts1.start();
        ts2.start();
        ts3.start();
        ts4.start();
